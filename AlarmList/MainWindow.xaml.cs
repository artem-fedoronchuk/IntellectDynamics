...
namespace AlarmList
{
    ...
    {
        private const Setup CurrentSetup = Setup.DefaultWindowsUserOnCorporate; 
        private enum Setup { BasicUserOnCorporate, WindowsUserOnCorporate, BasicUserOnEnterprise, WindowsUserOnEnterprise, DefaultWindowsUserOnCorporate, DefaultWindowsUserOnEnterprise };
        private const string DomainForBasicUser = "BASIC";
        private bool _isECode = false;
        private bool _isBasicUser = false;
        private int my_port_c_code_management_server_ssl = 443;
        ...
        private void SetAuthentication()
        {
            switch (CurrentSetup)
            {
                case Setup.BasicUserOnCorporate:
                    _userName = "";
                    _password = "";
                    _domainName = DomainForBasicUser;
                    _isBasicUser = true;
                    break;
                case Setup.WindowsUserOnCorporate:
                    _userName = "";
                    _password = "";
                    _domainName = "";
                    break;
                case Setup.BasicUserOnEnterprise:
                    _userName = "";
                    _password = "";
                    _domainName = DomainForBasicUser;
                    _isBasicUser = true;
                    _isECode = true;
                    break;
                case Setup.WindowsUserOnEnterprise:
                    _userName = "";
                    _password = "";
                    _domainName = "";
                    _isECode = true;
                    break;
                case Setup.DefaultWindowsUserOnCorporate:
                    ...
                    break;
                case Setup.DefaultWindowsUserOnEnterprise:
                    _userName = CredentialCache.DefaultNetworkCredentials.UserName;
                    _password = CredentialCache.DefaultNetworkCredentials.Password;
                    _domainName = CredentialCache.DefaultNetworkCredentials.Domain;
                    _isECode = true;
        ...
        private void Login()
        {
            ...
            if (_isBasicUser)
            {
                _basicLogin = new BasicConnection(!_isECode, _userName, _password, _hostName, _isECode ? _port : my_port_c_code_management_server_ssl);
                try
                {
                    _loginInfo = _basicLogin.Login();
                ...
                _ntlmLogin = new NtlmConnection(_domainName, AuthenticationType.Windows, _userName, _password, !_isECode, _hostName, _port);

                try
                {
                    _loginInfo = _ntlmLogin.Login();
                    _token = _loginInfo.Token;
                    _timeToLive = (long) _loginInfo.TimeToLive.TotalMilliseconds;
                }
                catch (Exception ex)
